...
// ── City registry ────────────────────────────────────────────────────────────
// Each entry: [tab code, display name, region]
const CITIES = [
  // Northeast
...
const S = {
  ...
  iframe: {
    width: "100%",
    height: "1160px",   // tall enough for both charts; scrolls internally if needed
    border: "none",
    display: "block",
    background: "#0d0d0d",
  },
...
export default function ENSOCharts() {
  const [selected, setSelected] = useState(CITIES[0]);

  // The HTML files live in /public, so Vite serves them at the root
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.frameWrap}>
        <iframe
          key={src}          /* forces remount on city change so charts reinit cleanly */
          src={src}
          style={S.iframe}
